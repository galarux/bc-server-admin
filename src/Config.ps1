...
function ConvertTo-BcsaHtmlReport {
    ...
    $text = @{
        en = @{ search = 'Search setting or value...'; settings = 'settings'; generated = 'Generated'; by = 'Generated by'; company = 'a free tool by' }
        es = @{ search = 'Buscar ajuste o valor...'; settings = 'ajustes'; generated = 'Generado'; by = 'Generado con'; company = 'herramienta gratuita de' }
    }[$Language]
    ...
    # The report is a standalone file, so the company logos travel inside it.
    $imageRoot = Join-Path $PSScriptRoot '..\web\img'
    $logo = [Convert]::ToBase64String([IO.File]::ReadAllBytes((Join-Path $imageRoot 'galarux-logo.png')))
    $logoWhite = [Convert]::ToBase64String([IO.File]::ReadAllBytes((Join-Path $imageRoot 'galarux-logo-white.png')))
    $company = $script:BcsaCompany
...
<style>
:root { --bg:#f5f3f4; --card:#fff; --text:#1f1a1c; --muted:#6b6164; --line:#e4dcde; --head:#880015; --link:#880015; --val:#0b7a3b; --hover:#f8eced; }
@media (prefers-color-scheme: dark) { :root { --bg:#151113; --card:#1e191b; --text:#ece6e7; --muted:#a89ea1; --line:#3a3033; --head:#5e0d18; --link:#f08c9a; --val:#5fd08f; --hover:#2c2124; } }
...
<div class="wrap">
<header class="top">
<div><h1>$(& $enc $title)</h1><div class="meta">$meta</div></div>
<a class="brand" href="$($company.Url)"><picture><source media="(prefers-color-scheme: dark)" srcset="data:image/png;base64,$logoWhite"><img src="data:image/png;base64,$logo" alt="$($company.Name)"></picture></a>
</header>
<div class="toolbar"><input id="q" type="search" placeholder="$(& $enc $text.search)" autofocus><span id="count"></span></div>
$($sections.ToString())
<footer>$($text.by) <a href="$script:BcsaRepoUrl">BC Server Admin</a> $script:BcsaVersion &middot; $($text.company) <a href="$($company.Url)">$($company.Name)</a></footer>
</div>
<script>
(function () {
